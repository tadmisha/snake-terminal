...
def new_apple(snake, dangers, rows, cols) -> list[int]:
    #! Unavailables are the ones snake's body is in
    available_coords = [[x,y] for x in range(rows) for y in range(cols) if not [x,y] in snake and not [x,y] in dangers]
    apple = choice(available_coords)
    return apple


#& Prints the field
def print_field(rows: int, cols: int, snake: list[list[int]], apple: list[int], dangers):
    for y in range(cols):
        for x in range(rows):
            #! ■ for snake body, ◨/◧/⬒/⬓ for the snake head (the unfilled part looks in the direction), ● for the apple, . for empty cells
            print(f" {'■' if ([x,y] in snake[:-1]) else ("◧" if direction == 0 else '◨' if direction == 2 else '⬓' if direction == 1 else '⬒') if ([x,y] == snake[-1]) else '●' if [x,y] == apple else '@' if [x,y] in dangers else'.' }  ", end='')
        print('\n')
...
def main():
    ...
    dangers = [[x,y] for x in range(rows) for y in range(cols)]
    dangers = sample(dangers, int(sqrt(rows*cols)/1.3))
    ...
    apple = new_apple(snake, dangers, rows, cols)
    ...
    while run:
        ...
        print(f" SCORE: {score}\n")
        print_field(rows, cols, snake, apple, dangers)
        sleep(frame_delay)
        ...
        if new_head in snake or new_head in dangers:
            run = False
